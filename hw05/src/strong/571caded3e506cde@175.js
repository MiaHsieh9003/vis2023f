...
function _strong1(d3,output,drag,url_list,invalidation)
{
  // 指定圖表的尺寸。
  const width = 1000;
  const height = 500;
  ...
  // 計算圖形並啟動力模擬。
  const root = d3.hierarchy(output.children[5]);
...
export default function define(runtime, observer) {
  ...
  const fileAttachments = new Map([
    ["simple(分數版).json", {url: new URL("../simple.json", import.meta.url), mimeType: "application/json", toString}],
  ...
  main.variable(observer("strong1")).define("strong1", ["d3","output","drag","url_list","invalidation"], _strong1);
