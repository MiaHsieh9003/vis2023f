...
function _artist_Column1(artist,innerCircleQuestion){return(
artist.map(row => row[innerCircleQuestion])
)}
...
function _artist_Column2(artist,outerCircleQuestion){return(
artist.map(row => row[outerCircleQuestion])
)}
...
function _10(scatterplotData){return(
Object.values(scatterplotData[5])[0]
)}

function _11(htl){return(
htl.html`<h2>Simple baseline:</h2><br>
<h4>完成頁面路徑</h4>`
)}

function _breadcrumb1(d3,breadcrumbWidth,breadcrumbHeight,sunburst,breadcrumbPoints,color)
{
  const svg = d3
    .create("svg")
...
htl.html`<h2>Medium baseline</h2>`
)}
...
<h4>從上圖中，我們可以看出：
  <ul>
    <li>替代空間的藝術工作者占此問卷的第二名</li>
    <li>不論是什麼形態的藝術工作者主要都採取減少包裝和再生材料來減少碳排放量</li>
    <li>非營利組織的藝術工作者除了減少包裝外也有一定的比例找符合永續精神的場地來達成減碳目標</li>
  </ul>
</h4>`
)}
...
d3
  ...
  .range(["#8E354A","#2A9563","#E16B8C","#63DCA4","#0D5661",,"#DC9FB4","#0D5661","#33A6B8","#81C7D4"])
  .unknown("#BEBEBE")
)}
...
htl.html` <hr><h2>Strong baseline:</h2><br>
  <h4>按"下一個數據"按鈕就會同時輸出該藝術工作者選擇的:「工作地點」與「環保選擇」的輸出in console，兩者互動: 工作地點vs環保選擇</h4>`
)}

function _count(Inputs){return(
Inputs.button("下一個數據")
)}

function _replay(Inputs){return(
Inputs.button("工作類型與環保選擇關係")
)}

function _addc(scatterplotData){return(
function addc(count)  {
  var output,output2;
  output = Object.values(scatterplotData[count])[0];
  output2 = Object.values(scatterplotData[count])[1];
...
function* _progress(replay,DOM,addc,count)
{
  replay;
  const width = 360;
  const height = 20;
  const context = DOM.context2d(width, height);
  context.canvas.style.border = "solid 5px purple";
...
export default function define(runtime, observer) {
  ...
  main.variable(observer("artist_Column1")).define("artist_Column1", ["artist","innerCircleQuestion"], _artist_Column1);
  main.variable(observer("outerCircleQuestion")).define("outerCircleQuestion", ["artist"], _outerCircleQuestion);
  main.variable(observer("artist_Column2")).define("artist_Column2", ["artist","outerCircleQuestion"], _artist_Column2);
  ...
  main.variable(observer()).define(["scatterplotData"], _10);
  main.variable(observer()).define(["htl"], _11);
  main.variable(observer("breadcrumb1")).define("breadcrumb1", ["d3","breadcrumbWidth","breadcrumbHeight","sunburst","breadcrumbPoints","color"], _breadcrumb1);
  ...
  main.variable(observer("viewof count")).define("viewof count", ["Inputs"], _count);
  ...
  main.variable(observer("addc")).define("addc", ["scatterplotData"], _addc);
  main.variable(observer("progress")).define("progress", ["replay","DOM","addc","count"], _progress);
  return main;
}
